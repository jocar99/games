...
function PlayState:enter(params)
    ...
    self.board:checkIfMatchesExist()
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            for k, tile in pairs(match) do
                local addBool = true
                for k, tile2 in pairs(self.scoringTiles) do
                    if tile2 == tile then
                        addBool = false
                        break
                    end
                ...
                if addBool == true then
                    table.insert(self.scoringTiles, tile)
                end
            end
        end
        
        for k, tile in pairs(self.scoringTiles) do
    ...
    -- if no matches, we can continue playing
        ...
        self.board:checkIfMatchesExist()
        if self.board.mathcesFound == 0 then
